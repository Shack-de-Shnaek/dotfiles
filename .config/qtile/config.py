...
def autostart():
    ...
    startup_program = os.path.expanduser("~/.config/qtile/scripts/startup_program.sh")
    ...
    subprocess.call(["sh", startup_program], timeout=5, start_new_session=True)
...
keys = [
    ...
    # rofi
    Key(
        [mod], "d", lazy.spawn("rofi -show drun -config ~/.config/rofi/rofidmenu.rasi"), desc="Spawn rofi in dmenu mode"
    ),
    Key(
        [mod, "shift"],
        "d",
        lazy.spawn("rofi -show run -config ~/.config/rofi/rofidmenu.rasi"),
        desc="Spawn rofi in window mode",
    ),
    Key(
        [mod],
        "t",
    ...
    Key(
        [mod],
        "c",
        lazy.spawn('rofi -modi "clipboard:greenclip print" -show clipboard -config ~/.config/rofi/rofidmenu.rasi'),
        desc="Show clipboard manager",
    ),
    Key([mod], "period", lazy.spawn('rofimoji --selector-args="-config ~/.config/rofi/rofidmenu.rasi"')),
    # screens
...
layouts = [
    ...
    layout.MonadWide(
        border_focus_stack=[PRIMARY, SECONDARY], border_width=2, margin=4, min_ratio=0.85, max_ratio=0.95, ratio=0.85
    ),
    layout.Tile(),
    layout.Max(),
...
def spawn_program(program_command):
    ...
    if program_command in ["btop"]:
        command_arguments = [terminal] + command_arguments
